...
tokenizer = Wav2Vec2CTCTokenizer(folder_model + "/vocab.json", unk_token="[UNK]", pad_token="[PAD]", word_delimiter_token="|")
...
def merge_repeats(path, output):
    ...
    for item in output.char_offsets[0]:
        ...
        segments.append(SegmentTest(
            item['char'], path[i1].time_index, path[i2-1].time_index + 1, score))
    return segments


def merge_words(segments, time_per_offset, start_time, separator=' '):
    words = []
    words_default = []
    i1, i2 = 0, 0
    while i1 < len(segments):
        if i2 >= len(segments) or segments[i2].label == separator:
            if i1 != i2:
                ...
                words.append(SegmentTest(word, segments[i1].start, segments[i2-1].end, score, segments[i1].start * time_per_offset + start_time, segments[i2-1].end*time_per_offset + start_time).serialize())
                words_default.append(SegmentTest(word, segments[i1].start, segments[i2-1].end, score, segments[i1].start*time_per_offset + start_time, segments[i2-1].end*time_per_offset + start_time))
            i1 = i2 + 1
            i2 = i1
        else:
            i2 += 1
...
def speech_to_text(input_file, start_time, end_time):
    ...
    word_segments, reliability_transcript = merge_words(segments, time_per_offset, start_time)
...
if __name__ == "__main__":
    ...
    transcript, reliability_transcript, word_segments = speech_to_text(input_file, 0, 1)
    print(transcript)
